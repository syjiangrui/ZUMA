# HTML 移动端祖马项目方案

## 1. 项目目标

构建一个适合手机浏览器游玩的 HTML 祖马游戏原型，优先保证：

- 竖屏手机体验
- 核心玩法可玩
- 球体沿轨道移动稳定
- 球体具备可见纹理滚动效果
- 后续易于扩展关卡、特效、音效和 UI

当前阶段不追求商业级美术资源和完整内容量，先完成一个稳定、可迭代的技术底座。

## 2. 核心需求

### 必须实现

- 单页面 HTML 游戏
- `Canvas 2D` 渲染
- 一条固定曲线路径
- 球链沿路径推进
- 炮台瞄准与发射
- 发射球插入球链
- 三消与连锁消除
- 胜负判定
- 触屏操作可用
- 球体带纹理，并有明显滚动动画

### 后续扩展

- 多关卡配置
- 特殊球
- 音效与背景音乐
- 粒子特效
- 暂停/继续
- 本地最高分
- PWA 支持

## 3. 技术方案

### 技术栈

- `HTML`
- `CSS`
- `JavaScript`
- `Canvas 2D`

第一阶段先不引入框架和构建工具，直接用原生方式完成原型。原因：

- 原型开发速度快
- 调试简单
- 移动端运行链路短
- 便于先验证玩法和渲染结构

如果后续项目复杂度提升，再切换到 `Vite + 模块化 JavaScript` 或 `TypeScript`。

## 4. 核心架构

### 4.1 游戏模块划分

- `Game`: 游戏主循环、状态切换、全局更新和渲染
- `Path`: 路径定义、采样、按弧长取点
- `Chain`: 球链推进、插入、闭合、消除检测
- `ChainState`: 链段分裂、前段停驻、后段追赶、重新并链
- `Shooter`: 炮台朝向、当前球、下一球、发射逻辑
- `Projectile`: 飞行中的球
- `Renderer`: 背景、轨道、球体、UI 绘制
- `Input`: 触摸与鼠标统一输入
- `Assets`: 纹理生成或纹理加载

### 4.2 核心数据模型

轨道球的基础位置仍然由路径距离驱动，但不能只保留 `s`。

建议数据模型：

```js
{
  id: 17,
  paletteIndex: 2,
  radius: 18,
  s: 320,
  rotation: 0,
  offset: 0,
  offsetMode: "idle",
  impact: 0
}
```

说明：

- `s` 表示球当前的路径距离
- `rotation` 表示球纹理自转角
- `offset` 表示插入让位、消除补位等过渡位移
- `offsetMode` 用来区分正常、插入、补位等过渡状态
- `impact` 用于碰撞和接合的瞬时视觉反馈

除单球状态外，还需要一层链条状态：

```js
{
  chainHeadS: 680,
  splitState: {
    index: 12,
    frontOffset: 0
  }
}
```

说明：

- `chainHeadS` 表示整条链的基础推进距离
- `splitState` 表示当前链是否被中段消除切成两段
- 当存在 `splitState` 时，前半段和后半段不再共享完全相同的运动规则

飞行中的球使用另一套数据：

```js
{
  paletteIndex: 1,
  radius: 18,
  x: 240,
  y: 600,
  vx: 320,
  vy: -540,
  rotation: 0
}
```

## 5. 轨道与滚动动画方案

### 5.1 轨道表示

球链不直接依赖屏幕坐标速度，而是依赖轨道距离。

实现方式：

- 先定义一组控制点
- 用曲线或折线采样成大量 `pathPoints`
- 每个采样点存储 `x / y / len`
- `len` 表示从轨道起点到该点的累计长度

这样球的位置通过 `s` 映射到轨道点，而不是单独做二维速度积分。

### 5.2 球链推进

球链推进需要拆成三层：

- 基础推进层：整条链沿路径持续向终点推进
- 过渡层：插入让位、消除补位、局部冲击等临时位移
- 链段层：中段消除后，链条可以暂时分裂成前后两段

正常状态下：

- 头球每帧增加基础路径距离
- 后续球保持接近一个直径的逻辑间隔

插入时：

- 新球先以碰撞点附近的位置并入轨道
- 后半链条先让位，再逐步回到标准间距
- 三消判定不应在插入当帧立刻完成，应在插入动作基本成立后再触发

中段消除时：

- 前半段应先停住，不继续向终点推进
- 后半段单独追赶前半段，速度应由专门的补链规则控制
- 两段真正接触后，才能重新合并成一条链
- 重新合并后再做跨接缝的连锁消除判定

这部分不是表现优化，而是祖马核心玩法的一部分。

### 5.3 球体纹理滚动

要实现“看得见的滚动”，球必须带方向性纹理。

采用方案：

- 使用 Canvas 动态绘制球纹理
- 纹理包含条纹、旋涡、高光与暗部
- 球的自转角由路径位移驱动

核心公式：

```js
ball.rotation = ball.s / ball.radius;
```

说明：

- 这是二维视觉模拟中的“无滑动滚动”近似
- 球沿轨道前进越远，纹理旋转越多
- 纹理朝向和球沿路径移动方向分离处理

### 5.4 渲染方式

每帧渲染流程：

1. 通过 `s` 计算球心位置
2. 保存画布状态
3. 平移到球心
4. 旋转到 `rotation`
5. 绘制球纹理
6. 恢复画布状态

## 6. 移动端优先设计

### 6.1 屏幕与布局

- 优先竖屏
- 画布按视口自适应缩放
- 使用固定逻辑分辨率 + CSS 缩放方案

建议逻辑尺寸：

- `430 x 932` 附近的竖屏比例

### 6.2 输入

- 支持 `pointer` 事件
- 点击/拖动瞄准
- 抬手或点击触发发射
- 同时兼容桌面鼠标调试

### 6.3 性能

- 路径采样结果预计算
- 球纹理离屏缓存
- 避免每帧创建大量对象
- 控制粒子数量
- 尽量只用 `Canvas 2D` 基础能力，不上重滤镜

## 7. 分阶段实施计划

### 阶段 1：基础原型

目标：先做出一个稳定可玩的最小版本。

交付内容：

- 基础页面和 Canvas
- 响应式移动端布局
- 固定轨道
- 球链自动推进
- 炮台瞄准
- 发射球飞行
- 球命中插入
- 插入让位动画
- 基础三消
- 消除后的基础断链与重新并链
- 纹理球滚动动画

### 阶段 2：玩法完善

目标：把当前原型补成一局完整可玩的游戏，并让玩法反馈开始接近真正游戏。

交付内容：

- `gameState` 与单局生命周期
- 失败/胜利状态
- 重开按钮与结束 UI
- 分数、连击与连锁统计
- 正式的连锁消除流程
- 将下一球预览纳入正式 HUD
- 具备可用版本的断链吸附/回拉手感

说明：

- 断链/并链手感的深度打磨不作为 Phase 2 的阻塞项
- 当一局游戏的规则闭环已经稳定后，允许先进入 Phase 3，再把手感优化作为长线事项持续迭代

### 阶段 3：表现优化

目标：提升观感和手机体验。

交付内容：

- 粒子和闪光
- 更好的球体纹理
- UI 皮肤
- 音效
- 低端机性能优化
- 持续性的断链/并链手感微调

### 阶段 4：内容扩展

目标：具备更完整的可玩性。

交付内容：

- 多关卡
- 特殊球
- 难度曲线
- 本地存档

## 8. 第一阶段目录建议

建议先使用最小目录结构：

```text
game/
  index.html
  style.css
  main.js
```

在第一阶段中，先把逻辑写在 `main.js`，等原型稳定后再拆分模块。

## 9. 第一阶段关键技术决策

### 决策 1：不用 DOM 做球体动画

原因：

- 球数量多时 DOM 成本高
- 路径运动和旋转更适合 Canvas
- 更容易统一做粒子和特效

### 决策 2：先用程序化纹理，不先依赖外部图片

原因：

- 能快速验证滚动效果
- 减少资源依赖
- 后续可替换成正式贴图

### 决策 3：先做单关卡固定路径

原因：

- 先验证球链、插入和消除逻辑
- 降低第一阶段复杂度

## 10. 主要风险与应对

### 风险 1：插入与消除逻辑不稳定

应对：

- 明确区分轨道球和飞行球
- 所有轨道运动统一用 `s` 驱动
- 在 `s` 之外保留过渡层状态，不要把所有动画都硬塞进一个位置值
- 三消判定要和插入动画、并链动画解耦

### 风险 2：断链状态处理错误

应对：

- 把断链看成一个明确状态，而不是若干速度参数
- 前半段停驻、后半段追赶、重新并链要有独立规则
- 连锁判定不能跨越尚未闭合的断口

### 风险 3：滚动动画看起来不明显

应对：

- 纹理必须具备方向性
- 加强高光、暗纹和条纹对比
- 必要时叠加外圈装饰环

### 风险 4：移动端分辨率适配不稳

应对：

- 使用固定逻辑分辨率
- 用统一缩放策略映射到实际屏幕
- 先优先适配常见竖屏比例

### 风险 5：手感问题被误判为单纯数值问题

应对：

- 先判断是状态机问题还是参数问题
- 补位速度、停驻逻辑、并链时机要先正确，再做手感调参
- 不要试图只靠一个速度常量修复所有违和感

## 11. Phase 2 执行顺序

当前 Phase 1 已具备：

- 中央发射口与环绕轨道
- 发射、插入与基础三消
- 中段消除后的基础断链与重新并链
- 纹理球滚动动画

因此 Phase 2 按以下顺序推进：

1. 建立 `gameState` 与单局生命周期
2. 接入正式失败/胜利判定
3. 补齐 HUD、结束提示与重开按钮，并把下一球预览纳入正式 UI
4. 建立分数、连击与连锁事件流
5. 完成正式的连锁消除规则
6. 在规则稳定后打磨断链吸附/回拉手感

## 12. 当前建议

当前原型已经证明：项目不仅需要“路径推进”，还需要“链段状态管理”和“单局状态闭环”。

因此 Phase 2 应遵守三个原则：

- 先补状态闭环，再补手感
- 计分、连击和连锁不要散落在渲染逻辑里，应由消除事件驱动
- 断链回拉的优化要建立在胜负、重开和连锁判定已经稳定的前提上

Phase 2 的成功标准应当是：

- 玩家能完整开始、进行并结束一局游戏
- 每次消除和连锁都有明确反馈
- 断链闭合与跨缝连锁行为可预测
- 出现问题时能通过重开快速复现与调试

在 Phase 2 中，应优先验证以下逻辑正确性：

- `playing / win / lose` 是否正确控制更新循环
- 胜负判定是否不会与连锁判定互相打架
- 分数与连击是否来自同一套消除事件，而不是从 UI 反推
- 断链重新接合后，是否只在真实闭合后触发跨缝连锁

## 13. Phase 2 详细任务拆解

### 13.1 目标与边界

Phase 2 的核心目标不是继续堆原型功能，而是把当前系统补成“可以完整打一局”的结构。

纳入范围：

- 单局状态机
- 胜负判定
- 重开流程
- HUD 与结束反馈
- 分数、连击、连锁统计
- 正式连锁消除规则
- 断链吸附/回拉手感打磨

暂不纳入：

- 音效与粒子
- 多关卡与特殊球
- 本地存档
- 复杂暂停系统

### 13.2 工作包 A：单局状态机与生命周期

目标：把当前“原型循环”升级成“有开始、进行、结束、重开”的回合生命周期。

任务：

- 引入统一的 `gameState`
- 明确 `playing / win / lose` 三种主状态
- 让链条推进、发射球更新、碰撞检测只在 `playing` 下运行
- 提供统一的重置入口，复用当前 `createChain()`、发射器重置和临时状态清理逻辑

验收标准：

- 游戏结束后球链、发射球和输入不会继续推进
- 不刷新页面也能完整重开一局
- 不会出现上一局的 `splitState`、`pendingMatchChecks` 或发射球残留到下一局

说明：

- `paused` 不是当前阻塞项，可以先不纳入 Phase 2

### 13.3 工作包 B：失败/胜利判定与回合出口

目标：把当前原型里“越线后直接重建球链”的行为，升级成正式的胜负结果。

任务：

- 把当前 `tailS > totalPathLength + EXIT_GAP` 的原型重置逻辑改为 `lose`
- 定义 `win` 条件：球链清空，且没有飞行球、没有待处理连锁检查
- 区分“胜负判定成立”和“真正进入结束状态”的时机，避免和插入/连锁同帧冲突

验收标准：

- 球链越线后进入失败状态，而不是立即开始新局
- 最后一组球消失时，不会因为还有飞行球或待结算连锁而误判胜利
- 胜负状态能稳定显示到玩家主动重开

说明：

- 第一版可沿用当前原型的越线语义，先把结果状态做正确；是否进一步改成更像原作的吞口判定，可留到后续微调

### 13.4 工作包 C：HUD、下一球预览与重开入口

目标：把已有的原型信息显示整理成正式 HUD，并补齐结束 UI。

任务：

- 整理顶部信息区，至少包含分数、连击和当前状态提示
- 将下一球预览从“原型可见元素”升级为正式 HUD 组成部分
- 提供可点击/可触控的重开按钮
- 增加 `win / lose` 的结束提示层

验收标准：

- 玩家无需看控制台即可理解当前状态
- 下一球预览在整局中始终与实际出球一致
- 结束后可通过按钮快速重新开始

### 13.5 工作包 D：分数、连击与事件模型

目标：不要把计分写成若干临时加法，而是建立稳定的玩法事件流。

任务：

- 为每次发射建立一次“行动上下文”，用于归并后续消除与连锁
- 为一次消除生成统一事件，至少包含消除数量、触发来源、是否跨缝、所属行动
- 由消除事件驱动分数增加、连击层数和 HUD 提示
- 明确“基础消除分”和“连锁加成分”的计算规则

验收标准：

- 分数变化只来自消除事件，不从 `chain.length` 差值反推
- 同一发球引发的连续消除能累积为连击
- 新的一发球会重置上一轮的连击上下文

说明：

- 这一步的关键不是 UI，而是先把“计分依据是什么”定义清楚

### 13.6 工作包 E：正式连锁消除规则

目标：把当前基础三消升级成完整的“首消 -> 断链 -> 重新接合 -> 再判定”的规则链。

任务：

- 明确一次发射后的首轮消除判定
- 明确断链重新接合后的二次判定
- 只允许在真实闭合后跨断口触发连锁
- 将连锁次数和工作包 D 的事件模型打通

验收标准：

- 中段消除后，两段未接上时不会跨缝误判
- 两段接合后如果颜色成立，会触发第二轮及后续连锁
- 连锁次数与实际发生的消除轮次一致

### 13.7 工作包 F：断链吸附/回拉手感打磨

目标：在规则已经稳定的前提下，把断链后的追赶与闭合做得更像祖马。

任务：

- 重新审视后半段追赶速度，避免明显快于正常滚动速度
- 调整接缝闭合阈值与吸附时机，减少“突然吸上去”的感觉
- 必要时为接缝闭合增加轻微冲击或回弹，但不改变底层判定
- 把这一层优化限制在 `splitState` 及其相关过渡，不再引入新的大状态机实验

验收标准：

- 前半段停驻、后半段追赶、重新并链三个阶段清晰可感
- 后半段不会在接合前表现出明显失真的超速
- 闭合后连锁判定仍然稳定，不因手感优化引入新 bug

### 13.8 推荐实施顺序

推荐按以下工作包顺序执行：

1. 工作包 A：先补生命周期
2. 工作包 B：再补胜负出口
3. 工作包 C：让调试和试玩有正式入口
4. 工作包 D：建立计分事件流
5. 工作包 E：完成正式连锁
6. 工作包 F：最后打磨断链手感

原因：

- A 到 C 先解决“一局是否成立”
- D 和 E 再解决“奖励反馈是否正确”
- F 最后才处理“手感是否像原作”

## 14. 当前进度（2026-04-18）

### Phase 2 状态

- 工作包 A：已完成
- 工作包 B：已完成
- 工作包 C：已完成
- 工作包 D：已完成
- 工作包 E：已完成基础版本
- 工作包 F：已完成可用版本，剩余优化后置

### 已完成内容

- 已引入统一的 `gameState`，并用 `playing / win / lose` 管理单局生命周期
- 已提供统一的 `resetRound()` 重开路径，能够清理断链、延迟判定和飞行球等临时状态
- 已将越线逻辑改为正式失败状态，并加入清空球链后的胜利状态
- 已完成手机端可用的 HUD、下一球预览、右上角重开按钮和结束面板重开按钮
- 已建立基于 action context 的计分与连击事件流，不再从球链长度变化反推分数
- 已补齐基础连锁规则：首消、断链、重新接合、跨缝再判定
- 已将接缝闭合后的复查扩展为双侧复查，降低漏判连锁的风险
- 已将断链追赶改为“冻结 `chainHeadS`，后半段仅靠 `close` offset 追赶”，避免闭合前出现明显超速
- 已为断链闭合加入基于 `initialGap` 的前链整体回拉、并链姿态吸收到 `chainHeadS` 的收口，以及极短的 `merge settle` 缓冲
- 已修复”可见头球被消除后，剩余可见球突然高速追向旧目标位置”的问题：现在会裁掉已越界 leader balls，并把共有位移吸收到 `chainHeadS`
- 已将前链回拉改为完全刚性平移（所有前段球获得相同偏移），消除回拉过程中球间距被拉开的视觉问题；同时将前段回拉速度提升至与后段追赶一致（RATIO=1.0），实现双向对称合拢

### 当前已知结论

- 规则层已经基本闭环，后续问题更可能来自“手感不对”而不是“判定缺失”
- 连击现在能够显示、累计，并在本局结束时保留最高连击摘要
- 手机端的关键操作路径已经成立，不再依赖键盘 `R`
- 断链追赶与并链收口已经有可用版本，当前剩余工作更适合作为长期微调项，而不是继续阻塞阶段推进
- 因此可以把 Phase 2 视为功能闭环完成，并开始进入 Phase 3

### 下一步

- 进入 Phase 3，优先处理粒子/闪光、球体纹理提升、UI 皮肤和音效
- 断链/并链手感继续作为长线打磨项，在不破坏规则层稳定性的前提下穿插微调

## 15. Phase 3 执行顺序

### 15.1 目标与边界

Phase 3 的核心不是继续加玩法规则，而是把当前已经成立的一局游戏做得更像“成品”。

这一阶段的重点是：

- 提升反馈强度与视觉辨识度
- 统一球体、轨道、HUD 的视觉语言
- 补齐音效与手机端体验
- 在不破坏规则层稳定性的前提下，做必要的性能治理

明确不纳入本阶段主线的内容：

- 多关卡
- 特殊球
- 难度曲线
- 本地存档
- 大幅重写断链/并链规则

说明：

- `splitState` 相关的手感微调在 Phase 3 中仍然允许继续进行，但只作为穿插的小步迭代，不作为新的主线工作包阻塞其他表现优化任务
- Phase 3 的实现原则是“优先叠加表现层，不回头改大规则”

### 15.2 推荐实施顺序

推荐按以下顺序执行：

1. 工作包 G：视觉方向与材质统一
2. 工作包 H：粒子、闪光与反馈特效
3. 工作包 I：HUD / 结束页 / 按钮皮肤升级
4. 工作包 J：音效系统与移动端音频策略
5. 工作包 K：性能治理与降级策略
6. 工作包 L：长线手感微调（并行穿插，不阻塞前 5 项）

原因：

- G 先定视觉方向，否则后续粒子和 UI 容易各做各的
- H 和 I 解决“打起来像不像游戏”
- J 解决“反馈像不像成品”
- K 放在中后段做，因为需要基于已经接近真实成品的负载来判断瓶颈
- L 持续存在，但不再主导阶段推进

## 16. Phase 3 详细任务拆解

### 16.1 工作包 G：视觉方向与材质统一

目标：让当前程序化原型从“功能可玩”升级成“视觉语言统一”的版本。

任务：

- 统一球体、轨道、场景底座和 HUD 的配色方向
- 升级球体程序化纹理，让颜色、亮面、暗面、条纹更像同一套材质
- 调整轨道材质，让其更接近祖马石质/祭坛语义，而不是单纯几条描边
- 统一字体、按钮圆角、描边、投影等视觉语法

验收标准：

- 球体、轨道、发射器和 HUD 不再像四套独立风格拼在一起
- 球体在移动和旋转时更容易看出材质与滚动方向
- 手机竖屏里第一眼就能看出“这是祖马风格的关卡场景”，而不是通用 Canvas 原型

说明：

- 这一阶段仍优先保留程序化纹理路线，不强制引入外部贴图

### 16.2 工作包 H：粒子、闪光与反馈特效

目标：补齐“命中、插入、消除、并链、胜负”这些关键事件的视觉反馈。

任务：

- 为发射命中增加短促闪光或冲击火花
- 为消除增加碎片、亮点或烟雾类粒子
- 为接缝重新并链增加轻量吸附/落地特效
- 为胜利/失败增加至少一层全屏级反馈，而不是仅靠文字
- 将特效分层控制在可开关、可降级的范围内

验收标准：

- 插入、三消、接缝连锁、胜负这几个关键事件都有明确视觉反馈
- 特效不会遮挡球链判读，也不会掩盖真正的玩法状态
- 在手机端上不会因为粒子过多导致明显掉帧

说明：

- 特效优先服务“状态可读性”，不是纯装饰

### 16.3 工作包 I：HUD / 结束页 / 按钮皮肤升级

目标：把当前“能用”的 UI 提升到更接近正式产品的层次。

任务：

- 重做顶部 HUD 的版式、层级和图标/装饰语法
- 优化下一球预览和重开按钮的视觉优先级
- 提升结束页的标题、分数摘要、按钮反馈和整体构图
- 补齐手机端按压态、按钮可读性和安全点击区

验收标准：

- HUD 能清晰表达状态，但不再像调试面板
- 结束页看起来像完整界面，而不是原型遮罩
- 手机端单手点击按钮时反馈明确、不易误触

说明：

- Phase 3 的 UI 仍然可以继续放在 Canvas 内完成，不强制切到 DOM

### 16.4 工作包 J：音效系统与移动端音频策略

目标：让“发射、命中、消除、连击、胜负”有明确声音反馈，同时保证手机端可用。

任务：

- 设计并接入最小音效集：发射、插入命中、消除、接缝连锁、胜利、失败
- 处理移动端音频解锁时机，确保首次交互后可以正常播放
- 增加静音或音量开关入口
- 明确短音效的复用和并发策略，避免同一时刻声音混乱

验收标准：

- 关键动作都有声音反馈，且不会明显延迟
- 手机浏览器首次交互后音效可稳定工作
- 声音层级清晰，不会因为连锁而变成噪音堆叠

说明：

- 背景音乐不是 Phase 3 必须项，优先保证短音效质量和触发时机

### 16.5 工作包 K：性能治理与降级策略

目标：在接近成品的表现负载下，仍保持手机端可玩。

任务：

- 识别当前高频开销：球体绘制、粒子数量、路径查询、HUD 绘制、音效并发
- 控制对象创建频率，减少瞬时垃圾回收压力
- 为粒子、闪光和阴影建立简单降级策略
- 为低端机保留“反馈完整但成本更低”的渲染模式

验收标准：

- 常见中端手机上不会因新增特效而明显卡顿
- 连续多次连锁、粒子叠加时仍能维持基本流畅
- 降级后依然保留玩法必要反馈，不会因为“优化”而丢失可读性

说明：

- Phase 3 的性能治理是“围绕表现层做减法”，不是重写整套底层

### 16.6 工作包 L：长线手感微调（并行项）

目标：继续迭代断链追赶、吸附、并链后的局部反馈，但不再作为阶段阻塞项。

任务：

- 继续微调 `splitState` 相关参数
- 在必要时微调并链后的冲击和缓冲时长
- 结合 Phase 3 新特效一起判断“观感像不像祖马”

验收标准：

- 每次微调都不破坏当前规则正确性
- 手感优化能够独立回退，不影响主线表现功能
- 不再出现为了微调手感而连带重写规则层的情况

说明：

- 这是长期并行任务，不要求在 Phase 3 某个固定时间点“彻底完成”

## 17. 2026-04-19 Phase 3 Complete

This addendum records the current visual-direction decisions without rewriting the older planning sections.

Current status:
- `Phase 3 / G` is complete.
- `Phase 3 / H` is in progress.
- Path geometry is intentionally frozen for now: off-screen entry plus Archimedean spiral remains the baseline.

What has already changed in code (G):
- Shooter replaced with a classic Zuma stone-frog: green-stone Mayan idol with moss-toned body, wider head/mouth, three head-crest ridges, Mayan zigzag decoration, warm amber round-pupil eyes (friendly, not menacing). Entire frog rotates with aim angle; ground shadow stays flat. 0.78× scale for better proportion. Procedural Canvas 2D, no external assets.
- Frog belly socket shows current ball color (same as mouth); HUD "下一个" shows the actual next ball.
- Shooter color generation now follows classic Zuma active-color behavior: current and next balls are sampled only from colors still present in the chain, and are revalidated after eliminations so extinct colors do not remain queued.
- HUD panels, restart button and end cards are moving toward a stone altar / bronze trim look.
- The track has been simplified toward a carved groove instead of a heavy framed pipe.
- Balls now use a stone-body render plus a moving symbolic belt instead of the old flat rotating badge.
- The belt texture is horizontally tileable and includes a seam-blend post-pass to reduce rotation seam flashes.

Important rendering decision:
- We tried a pseudo full-sphere texture projection and rejected it.
- Reason: it produced center stretching and a "sticker on the front" read.
- The current implementation intentionally uses a rolling equatorial band because it is less uncanny and more readable on a phone-sized ball.

What has already changed in code (H):
- Debris particle system added: each eliminated ball spawns 6 colored fragments (palette base/bright/accent) that fly outward with gravity, fade and shrink over ~0.5s. Capped at 120 particles total.
- Impact aura already existed from Phase 2 (short glow pulse on collision/merge).

What has already changed in code (H — complete):
- Victory: golden particle burst from center + rising sparkles for 1.5s + radial gold glow overlay.
- Defeat: screen shake (14px, ~0.3s decay) + red vignette flash (0.8s fade).
- Merge/seam closure flash deferred — low priority, revisit later.

What has already changed in code (J — complete):
- Procedural audio via `SfxEngine` class (Web Audio API, zero external files).
- Effects: shoot (sine pop + high-pass noise puff), hit (bandpass noise burst), match (dual-tone chime + noise crackle, pitch rises per combo level), win (C-E-G-C ascending arpeggio), lose (sawtooth rumble + low-pass noise).
- AudioContext lazily created on first user gesture for mobile compatibility.
- Default muted; HUD sound toggle button (speaker icon with red slash when muted).
- HUD layout adjusted: next-preview (44×44) + sound button (36×38) + restart (64×38), non-overlapping.

What has already changed in code (I — complete):
- HUD panel cache rebuilt: stone speckle texture (light+dark ellipse layers), Mayan zigzag gold trim along bottom edge, sun/altar icon decoration, sub-panel micro speckle.
- HUD text hierarchy: title 22px bold with manual offset shadow, subtitle 11px dimmer, status row with color dot indicator (green=playing, gold=win, red=lose), score numbers gold `#f5d872`, labels grey `#c8bfa8`.
- Buttons enhanced: 1px press-down offset, border stroke brightened (0.42→0.52 alpha), sound icon slightly larger.
- Next-ball preview: rotating dashed gold halo (`setLineDash` + time-based rotation), text replaced with arrow `▸`, brighter bronze border.
- End card redesigned: panel 300×240 (was 266×164), altar badge with double ring + sun (win) or crack lines (lose), title 32px bold with offset shadow, score as large 28px gold number on own line, combo badge in inset sub-panel, restart button 200×40 with pulsing translucent glow, Mayan zigzag trim at bottom, win=gold border / lose=red-copper border.
- Match feedback: score number with manual offset shadow, combo>2 triggers gold-flashing panel border.
- All `ctx.shadowBlur` calls removed (caused GPU performance spikes). Replaced with manual offset text rendering (draw dark at +1px, then light at original position) and expanded translucent rects for glow effects.
- All panel colors unified to scene-matching grey: `#7a8590` / `#636e78` (was grey-blue `#6e7880` / `#57626b`).
- 2026-04-19 HUD regression fix: the left title slab turned near-black because `fillRoundedRect()` was used to build a clip path inside `hudPanelCache`, which filled the slab with the default black fill before `clip()`. Added `traceRoundedRect()` and moved clip/stroke-only call sites to it.
- Phase 1 legacy DOM `.hud` element hidden (`display: none` in index.html) — was overlaying Canvas with near-black background.

What has already changed (K — performance, effectively complete):
- Major rendering-cache pass done in earlier work (static scene, ball base, frog layers, HUD panels).
- All `shadowBlur` eliminated in I pass.
- Particle hard cap at 120.
- Further K work would only be needed if low-end device testing reveals new bottlenecks.

Phase 3 status: **complete**. G/H/I/J/K all delivered. L (hand-feel micro-tuning) is a permanent background item.

Next expected move:
- Phase 4 (multi-level, special ball types, difficulty curves, save/load).

## 18. Phase 4 状态 (2026-04-19)

### Phase 4 完成状态

原 Phase 4 已拆分为两部分：
- **Phase 4**（已完成）：多关卡流程闭环
- **Phase 5**（待开始）：特殊球、扩展关卡、PWA、暂停系统

详细设计文档：`docs/superpowers/specs/2026-04-19-phase4-phase5-split-design.md`
实施计划：`docs/superpowers/plans/2026-04-19-phase4-multi-level.md`

### 工作包完成情况

- 工作包 M（关卡数据模型）：已完成 — `levels.js` 8 关配置
- 工作包 N（选关界面+切换）：已完成 — levelSelect 状态、Canvas 选关界面、关卡间导航
- 工作包 O（难度曲线）：已完成 — 8 关参数递进（球数 20→42，速度 52→86，颜色 3→5）
- 发射球颜色策略：已调整为经典祖玛式 active-color 规则，只从当前链上仍然存在的颜色中出球，避免生成已在棋盘上绝迹的死色
- 工作包 P（本地存档）：已完成 — `save.js` localStorage 持久化
- 工作包 Q（路径泛化）：已完成 — path.js 调度器 + 4 种路径生成器
- 工作包 R（路径分配+打磨）：已完成 — 8 关分配不同路径、缩略图预览、淡入淡出过渡、全通关画面

### 新增模块

- `levels.js` — 关卡配置数组（id、名称、球数、速度、颜色数、路径类型+参数、射手位置）
- `save.js` — localStorage 存档读写（版本化格式，支持未来迁移）

### 路径系统

path.js 已重构为调度器模式：
- `createPath(shooterX, shooterY, pathType, pathParams)` — 入口调度
- `finalizePath(sampled)` — 共享后处理（弧长参数化 + Path2D 缓存）
- 4 种生成器：spiral（螺旋）、serpentine（蛇行）、rectangular（回字）、zigzag（之字）

### 下一步

进入 Phase 5：特殊球系统框架、核心特殊球（爆炸球、变色球、倒退球、减速球）、扩展关卡、暂停系统、BGM、PWA 支持。
