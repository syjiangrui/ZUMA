...
关卡路径仍使用**二次贝塞尔曲线**定义，但编辑器现在支持先摆 `waypoints`，再一键自动拟合成二次贝塞尔链。这样手工只需要决定骨架和关键拐点，控制点由算法自动求解。
...
level-paths.json          ← 编辑器直接读写此文件
    │                        格式: [{name, curves:[{p1,cp,p2},...], waypoints:[{x,y},...], shooterPos}, ...]
    │
    │  fetch (游戏启动时)
    ▼
...
3. 调 `平滑` 和 `误差`，点击 `⟳ 一键拟合`。
4. 如果局部仍不满意，可以继续拖 waypoint，再拟合一次；或者切回曲线模式微调单个 control point。

拟合算法在 `path-fit.js`，核心不是“固定每两个点一段”，而是**带误差控制的自适应二次曲线分段**：
...
- 对大拐角先按角度阈值切 chunk，尽量保留“祖玛式”明确转弯，而不是全部抹成一条大弧

这样做的结果是：

- 直线和缓弯会自动合并成少量大段
- 真正的急转弯会自动加段
- 不需要人工摆 cp，但仍然能控制“最多允许偏离原骨架多少像素”
...
| 操作 | 说明 |
|------|------|
| Waypoint 模式 | 点击布点、拖拽、插点、删点，先搭骨架 |
| 一键拟合 | waypoint → 二次贝塞尔链；误差超限处自动分段 |
| 从曲线提取 | 把现有曲线抽成 waypoint 骨架，便于重做拐点 |
...
- Delete / Backspace：删除当前选中的 waypoint 或曲线
...
游戏侧 `path.js` 不需要知道 waypoint；它仍只消费拟合后的 `curves`。Waypoint 只是编辑器侧的骨架辅助数据。
